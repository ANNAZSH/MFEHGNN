...
def load_alldata(image_file, label_file, lid_file):
    image_data = sio.loadmat(image_file)
    label_data = sio.loadmat(label_file)
    lidar_data = sio.loadmat(lid_file)

    data_key = image_file.split('/')[-1].split('.')[0]
    label_key = label_file.split('/')[-1].split('.')[0]
    lidar_key = lid_file.split('/')[-1].split('.')[0]

    data_all = image_data[data_key]  # dic-> narray , KSC:ndarray(512,217,204)
    GroundTruth = label_data[label_key]
    lid_data = lidar_data[lidar_key]

    [nRow, nColumn, nBand] = data_all.shape
    [l_nRow, l_nColumn, l_nBand] = lid_data.shape
    print(data_key, nRow, nColumn, nBand)
    print(lidar_key, l_nRow, l_nColumn, l_nBand)


    data = data_all.reshape(np.prod(data_all.shape[:2]), np.prod(data_all.shape[2:]))  # (111104,204)
    l_data = lid_data.reshape(np.prod(lid_data.shape[:2]), np.prod(lid_data.shape[2:]))
    data_scaler = preprocessing.scale(data)  # (X-X_mean)/X_std,
    lidar_scaler = preprocessing.scale(l_data)
    Data_Band_Scaler = data_scaler.reshape(data_all.shape[0], data_all.shape[1],data_all.shape[2])
    l_data_Scaler = lidar_scaler.reshape(lid_data.shape[0], lid_data.shape[1],lid_data.shape[2])
    return Data_Band_Scaler, GroundTruth, l_data_Scaler # image:(512,217,3),label:(512,217)

def load_onealldata(image_file, label_file, lid_file):
    image_data = sio.loadmat(image_file)
    label_data = sio.loadmat(label_file)
    lidar_data = sio.loadmat(lid_file)
    ...
    data = data_all.reshape(np.prod(data_all.shape[:2]), np.prod(data_all.shape[2:]))  # (111104,204)
    data_scaler = preprocessing.scale(data)  # (X-X_mean)/X_std,
    lidar_scaler = preprocessing.scale(lid_data)
    Data_Band_Scaler = data_scaler.reshape(data_all.shape[0], data_all.shape[1],data_all.shape[2])

    return Data_Band_Scaler, GroundTruth, lidar_scaler # image:(512,217,3),label:(512,217)
