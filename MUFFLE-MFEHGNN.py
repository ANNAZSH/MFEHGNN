...
parser.add_argument("-d","--tar_input_dim",type = int, default = 64) # PaviaU=103；salinas=204
...
# target
parser.add_argument("-m","--test_class_num",type=int, default=11)
...
test_data = '../datasets/MUUFL/MUUFL_HSI.mat'
test_label = '../datasets/MUUFL/MUUFL_gt.mat'
lid_data = '../datasets/MUUFL/MUUFL_LIDAR.mat'
...
for iDataSet in range(nDataSet):
    ...
    for episode in range(20000):  # EPISODE = 90000
        ...
        # target domain few-shot + domain adaptation
            ...
            supports_HS = supports[:, :64, :, :]
            supports_lidar = supports[:,64:,:,:]
            querys_HS = querys[:,:64,:,:]
            querys_lidar = querys[:, 64:,:,:]
            # supports_lidar = supports_lidar.unsqueeze(1)
            # querys_lidar = querys_lidar.unsqueeze(1)
            # calculate features
        ...
        if (episode + 1) % 1000 == 0 or episode == 0:
            ...
            train_datas, train_labels = next(iter(train_loader))
            train_datas_HS = train_datas[:, :64, :, :]
            train_datas_lidar = train_datas[:, 64:, :, :]
            ...
            for test_datas, test_labels in test_loader:
                ...
                test_datas_HS = test_datas[:, :64, :, :]
                test_datas_lidar = test_datas[:, 64:, :, :]
...
utils.classification_map(hsi_pic[4:-4, 4:-4, :], best_G[4:-4, 4:-4], 24,  "classificationMap/H-MUUFL_MMHGNN_{}shot.png".format(TEST_LSAMPLE_NUM_PER_CLASS))
