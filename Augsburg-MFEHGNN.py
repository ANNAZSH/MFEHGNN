...
parser.add_argument("-d","--tar_input_dim",type = int, default = 180) # PaviaU=103；salinas=204
...
test_data = '../datasets/Augsburg/Augsburg_HSI.mat'
test_label = '../datasets/Augsburg/Augsburg_gt.mat'
lid_data = '../datasets/Augsburg/Augsburg_LIDAR.mat'
...
for iDataSet in range(nDataSet):
    ...
    # C_S = torch.Tensor(args.class_num, args.feature_dim)
    # C_S = torch.nn.init.orthogonal(C_S, gain=1).cuda()
    # C_S.requires_grad = True
    # # C_T = torch.Tensor(args.test_class_num, args.feature_dim)
    # # C_T = torch.nn.init.orthogonal(C_T, gain=1).cuda()
    # # C_T.requires_grad = True
    #
    # parameters = [C_S]
    # parameters += list(feature_encoder.parameters())
    # optimizer
    ...
    for episode in range(15000):  # EPISODE = 90000
        ...
        # target domain few-shot + domain adaptation
            ...
            supports_HS = supports[:, :180, :, :]
            supports_lidar = supports[:,180:,:,:]
            querys_HS = querys[:,:180,:,:]
            querys_lidar = querys[:, 180:,:,:]
            # supports_lidar = supports_lidar.unsqueeze(1)
            # querys_lidar = querys_lidar.unsqueeze(1)
            # calculate features
        ...
        if (episode + 1) % 1000 == 0 or episode == 0:
            ...
            train_datas, train_labels = next(iter(train_loader))
            train_datas_HS = train_datas[:, :180, :, :]
            train_datas_lidar = train_datas[:, 180:, :, :]
            ...
            for test_datas, test_labels in test_loader:
                ...
                test_datas_HS = test_datas[:, :180, :, :]
                test_datas_lidar = test_datas[:, 180:, :, :]
...
utils.classification_map(hsi_pic[4:-4, 4:-4, :], best_G[4:-4, 4:-4], 24,  "classificationMap/H-Augsburg_MMHGNN_{}shot.png".format(TEST_LSAMPLE_NUM_PER_CLASS))
